...
class Character(object):
    ...
    def playerFight(self, enemy):
        keepGoing = True
        while(keepGoing):
            print(f"""
    1.)Basic Attack to {enemy.name}(This is a basic attack that has a high chance of success but low damage)
            
    2.)Magic Attack to {enemy.name} (Mana Attacks do mass ammounts of damage and ignore armor but have a low chance of success and cost 20 Mp)
    
    3.)Heal yourself using healing magic(Recover {self.recover} hp but it will cost you 10 mana)             
    """)
